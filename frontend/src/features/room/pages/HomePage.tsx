...
export function HomePage() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      // PageTransitionのexitToで遷移開始
      exitToRef.current?.(`/room/${roomId}`);
    } catch {
  ...
  const handleRejoin = async () => {
    ...
    try {
      ...
      exitToRef.current?.(`/room/${lastRoom.roomId}`);
    } catch {
      setError('ルームが見つかりませんでした');
      setLoadingAction(null);
    }
  ...
  const handlePractice = () => {
    exitToRef.current?.('/practice');
  };

  return (
    <PageTransition>
      {({ isExiting, exitTo, contentVisible }) => {
        // exitTo関数をrefに保存してハンドラから使えるようにする
